...
def get_job_match_from_db(skill_title: str, keyword: str, db: Session):
    stmt = (
        select(Job)
        .join(Skills, Job.job_id == Skills.job_id)
        .where(
            and_(
                Skills.keyword == keyword.lower(),
                Skills.skill_title == skill_title
            )
        )
        .distinct(Job.job_id)
    )
...
def get_skills_from_db(keyword: str, db: Session):
    stmt = (select(Skills.skill_title, func.count(Skills.skill_title).label("count")).where(
        Skills.keyword == keyword.lower()).group_by(Skills.skill_title).order_by(desc("count")))
    return db.execute(stmt).mappings().all()
